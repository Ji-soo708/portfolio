...
**기획**
        `}
        skills={skills}
        aboutText={`카페에서 코딩하는 것을 즐기는 개발자들에게 카카오 지도를 통해 코딩하기 좋은 카페를 추천하는 서비스입니다. 

서비스 내에서 카페의 리뷰 및 댓글을 보여주어 사용자가 선호하는 형태의 카페를 쉽게 찾을 수 있도록 돕습니다.`}
      ...
      <div className="flex justify-center mb-4">
        <div className="p-5 w-full max-w-[940px] bg-white shadow-md rounded-[25px] mx-4 lg:mx-8">
          <ProjectContentBox
            subtitle={`[AOP와 ArgumentResolver 기능 이용하여 반복 코드 제거](https://ji-soo708.tistory.com/6)`}
...
이러한 문제를 해결하기 위해 **Spring AOP 기능**을 이용하여 전체 소스 코드에 퍼진 반복되고 있는 코드를 한 곳에 모아 별도의 기능으로 분리시켜주고 사용자가 원하는 시점에서 실행할 수 있도록 했습니다. 그리고 \`HandlerMethodArgumentResolver\`를 이용하여 \`@LoginId\` 어노테이션이 달린 파라미터에 자동적으로 token의 값을 할당시켜 반복적인 코드를 제거할 수 있었습니다.`}
          ...
          <ProjectContentBox
            subtitle="[동시성 이슈 해결을 위한 무결성 제약 조건 채택](https://ji-soo708.tistory.com/2)"
...
먼저 \`ExecutorService\`를 활용하여 멀티 스레드 환경에서 테스트를 진행하면서 발생할 수 있는 문제를 분석했습니다. 토이 프로젝트의 특성상 낙관적 락까지 고려하지 않아도 충분히 무결성 제약 조건만으로 문제를 해결할 수 있었습니다. 따라서, 가장 간단하면서도 효과적인 방법으로 데이터베이스에 유니크 제약 조건을 걸어 문제를 원활히 해결할 수 있었습니다.`}
          ...
          <ProjectContentBox
            subtitle={`[Redis를 이용한 Refresh Token 도입](https://ji-soo708.tistory.com/22)`}
...
이러한 점을 고려하여 본 프로젝트에서는 update의 조건에 걸리는 컬럼에 대해 인덱스를 생성하고 여러 개의 레코드를 수정하는 경우, 더티 체킹 대신 Update 벌크 연산을 활용하여 성능을 향상시켰습니다.`}
          ...
          <ProjectContentBox
            subtitle="[코드 커버리지를 위한 JaCoCo 및 SonarCloud 도입](https://ji-soo708.tistory.com/21)"
